...
const server = net.createServer((socket) => {
  ...
  clients.add(socket);
  console.log(`[Server] Client ${socket.id} connected from ${socket.remoteAddress}`);

  let buf = '';
  socket.on('data', (chunk) => {
    console.log(`[Server] Received ${chunk.length} bytes from ${socket.id}: ${chunk.substring(0, 100)}`);
    buf += chunk;
    let idx;
  ...
  // Note: 'hello' message is sent after successful handshake validation
  // See connectionHandlers.js handleHandshake()
});
