...
function handleJoinLobby(socket, msg) {
  ...
  if (l.password && l.password !== (msg.password || "")) return send(socket, { type: 'error', code: 'INCORRECT_PASSWORD', message: 'Incorrect password' });
...
async function handleStartGame(socket, msg) {
  ...
  if (l.raceMode === 'square' && l.mappackId) {
    ...
    try {
      // Create a shuffled array of track indices for random map assignment
      // This ensures maps are randomly distributed across the board each game
      const trackIndices = Array.from({ length: 64 }, (_, i) => i);
      // Fisher-Yates shuffle
      for (let i = trackIndices.length - 1; i > 0; i--) {
        const j = Math.floor(Math.random() * (i + 1));
        [trackIndices[i], trackIndices[j]] = [trackIndices[j], trackIndices[i]];
      }
      console.log(`[Chess] Shuffled track indices for random board layout`);

      // Assign maps to all 64 positions using shuffled indices
      for (let pos = 0; pos < 64; pos++) {
        const shuffledIndex = trackIndices[pos];
        const map = await fetchMapFromMappack(l.mappackId, shuffledIndex);
        boardMaps.push({
          tmxId: map.tmxId,
          mapName: map.name,
          tagIds: map.tagIds || '' // Raw comma-separated tag IDs for client-side resolution
        });
        // Log first few assignments
        if (pos < 3 || pos === 63) {
          console.log(`[Chess] Position ${pos} (track ${shuffledIndex}): ${map.name} (TMX ${map.tmxId}), tagIds: "${map.tagIds}"`);
        }
      }
      game.boardMaps = boardMaps;
      console.log(`[Chess] Successfully assigned ${boardMaps.length} maps to board (randomized)`);
      // Log a summary
...
// Handle kick_player message
function handleKickPlayer(socket, msg) {
  const l = lobbies.get(msg.lobbyId);
  if (!l) return;
  if (l.host !== socket) return; // Only host can kick

  const playerName = msg.playerName;
  const playerIndex = l.playerNames.indexOf(playerName);
  if (playerIndex === -1 || playerIndex === 0) return; // Can't kick non-existent player or host

  const kickedSocket = l.players[playerIndex];

  // Remove player from lobby
  l.players.splice(playerIndex, 1);
  l.playerNames.splice(playerIndex, 1);
  l.open = l.players.length < 2;
  ...
  broadcastLobbyList();
  console.log(`[Lobby] Player ${playerName} kicked from lobby ${l.id}`);
}
