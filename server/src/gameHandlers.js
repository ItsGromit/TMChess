...
async function handleMove(socket, msg) {
  ...
  // Check if this is a pawn promotion (pawn moving to rank 1 or 8)
  const sourceSquare = game.chess.get(from);
  const isPromotion = sourceSquare && sourceSquare.type === 'p' &&
    (to[1] === '8' || to[1] === '1');
  ...
  if (isCapture) {
    ...
    const challenge = {
      from,
      to,
      promotion: isPromotion ? null : promotion, // Don't set promotion yet if it's a promotion capture
      isPromotion,
      tmxId: map.tmxId,
      mapName: map.name,
      attacker: socket,
