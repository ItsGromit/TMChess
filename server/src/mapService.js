...
// Tags come as comma-separated numeric IDs from the /api/maps endpoint (e.g. "1,3,14")
// These are resolved to names/colors client-side using hardcoded tag definitions
      ...
      res.on('end', () => {
        try {
          ...
          if (!response.Results || !Array.isArray(response.Results)) {
            console.error('[Chess] Mappack returned invalid format');
          ...
          for (let i = 0; i < Math.min(3, tracks.length); i++) {
            console.log(`[Chess] Track ${i}: ${tracks[i].Name} (MapId: ${tracks[i].MapId}), Tags: "${tracks[i].Tags || ''}"`);
          }
...
async function fetchMapFromMappack(mappackId, position) {
  ...
  // Fetch the entire mappack (uses cache if available, includes detailed tags)
  ...
  const map = {
    ...
    tagIds: Array.isArray(track.Tags) ? track.Tags.map(t => t.TagId).join(',') : (track.Tags || '')
  };

  // Debug: Log tags for first few positions
  if (position < 3) {
    console.log(`[Chess] Position ${position}: ${map.name} (TMX ${map.tmxId}), tagIds: "${map.tagIds}"`);
  }
